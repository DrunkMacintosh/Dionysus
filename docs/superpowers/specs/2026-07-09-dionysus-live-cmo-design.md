...
**Date:** 2026-07-09
**Status:** DRAFT v3 — full-design review applied (2026-07-09): wedge-first build inversion (D26), identity & custody redesign (D27), mechanical contracts (D28–D30); awaiting user review
**Scope:** Two-phase product. **Phase W (the wedge):** a sellable plan-first AI CMO on the existing Next.js pipeline — case-led strategy, drafts, verified send, attribution — shipped in weeks and validated with paying founders. **Phase P (the platform):** the live agentic CMO on the Hermes Agent runtime described in §1–§17, built when W's revenue validates it. The deterministic step-engine and hand-rolled Stage-1 orchestration/memory layers are still superseded — but by W's slimmer pipeline first, and the Hermes department second.
...
## 0. Stage W — the wedge (sellable v1, D26)

**Positioning:** *the plan-first AI CMO.* Okara gives founders a queue of chores; Dionysus gives them the campaign — how five real companies actually won (with sources), a route to their objective, this week's drafts, and proof of what worked. Runs entirely on the existing Next.js pipeline + Prisma + magic-link (Stage-1 row tenancy) — **no Hermes, no containers, no shared MCP server, no department infrastructure**. Employee names (Historian, Strategist, Copywriter…) appear in the UX as the faces of pipeline steps; the platform later makes them real agents with no founder-visible migration.

**W1 — lean core (~6 weeks; the validation clock starts here):**
1. **Intake** — product URL → `read_product` + brand kit (existing plumbing).
2. **Case brief** — 5 source-cited cases (EXTRACTED/INFERRED discipline, §6.2) + modernized plan + insight each. **Mid-tier model + citation-entailment check** (fetch each EXTRACTED source; a checker confirms the source supports the claim).
3. **Objective + Route** — founder sets a measurable objective; system proposes waypoints; founder iterates and commits (`Objective`/`Route`/`RouteWaypoint`).
4. **Drafts** — channel-native drafts for the active waypoint (X, LinkedIn, Reddit, HN, PH) with the §3 verify-checks as code assertions. Everything is **always-ask** (no autonomy tiers in W).
5. **Verified send — assisted-manual everywhere:** founder posts, pastes the public URL; hardened `verify_post` (§security addendum) confirms platform domain + content match.
6. **Attribution timeline** — `/r/[slug]` clicks + founder-reported outcomes → "this action drove X, so the next step changed" + a next-action recommendation (a pipeline step, the §6.7 recommender's W-form).
7. **Billing** — ≥$99/mo, credit-metered from day one (D23; `CreditLedger`).

**W2 — earn-the-demo additions (~4–5 weeks, behind W1):**
- **GA4 / GSC read-only analytics** (D21) — real conversions replace founder reports in the attribution beat.
- **Connected-API posting for X (and LinkedIn once tier-approved)** — approve → auto-publish on owned feeds (D9 default). This pulls forward the **token-broker slice of D27(3)**: scoped `broker.publish`, per-business envelope encryption, no raw tokens in pipeline code. LinkedIn's legal-entity + tier approval starts at W1 kickoff (calendar risk).
- **Simulator-lite** — one cheap "community reaction check" beside community-channel drafts, labeled *prediction* (D14 discipline).

**Validation gate (blocks all platform stages):** within ~90 days of W1 shipping — ≥5–10 founders paying ≥$99/mo, retained through month 2, draft edit-distance trending down (D22's metric, instrumented from W1). Pass → build the platform (§1–§17) staged below. Fail → pivot or kill; the department would not have saved it.

**What W deliberately defers:** the live department (persistent employees, self-improving skills), always-on social radar, chat surfaces (cockpit-only in W), memory graph + graphify (a flat per-business learnings table suffices), autonomy tiers, video, Outreach, CRO, SEO/AEO. Deferring the multi-tenant agent platform also defers most of the CRITICAL security surface (D27) — the addendum below is the platform's entry contract, not W1's blocker (except `verify_post` hardening and W2's token broker, which W uses).

---

*Everything below this line is the **platform** (Phase P) — the destination architecture, built after the W gate passes.*
...
Three risks no amount of spec-writing resolves. **D26 re-sequences how they're bought down:** Stage W validates the PMF risk with revenue *before* a dollar is spent on the framework bet, and the moat risk's reframe (attribution visibility) ships in W1 — only the Hermes risk remains a platform-stage gamble, taken after the product is proven. Recorded so they're chosen knowingly, not stumbled into.
...
## 17. Build order (v3 — wedge first, platform earns in)
...
**W1 — the wedge core (~6 weeks; see §0).** Case brief (mid-tier Historian pipeline + entailment check) → objective/route → drafts → assisted-manual verified send (hardened `verify_post`) → click attribution + timeline → Stripe at ≥$99/mo credit-metered. Existing pipeline, existing tenancy, cockpit-only.

**W2 — earn-the-demo (~4–5 weeks, behind W1).** GA4/GSC read-only analytics (D21); connected-API posting for X (+LinkedIn when tier-approved) with the token-broker slice of D27.3; Simulator-lite (labeled prediction on community drafts). LinkedIn entity/tier application filed at W1 kickoff.

**⛔ Validation gate (D26).** ≥5–10 founders paying ≥$99/mo, retained through month 2, edit-distance trending down, within ~90 days of W1. No platform stage starts before this passes.

**Platform stages (post-gate; the §1–§16 architecture):**

1. `dionysus-mcp` server wrapping the (now W-proven) TS plumbing + persistence with **D27.1 ambient identity from day one** (per-container credentials; no businessId parameters); the **D28 LLM gateway**; ledger-measure real per-business cost against the envelope.
2. Coordinator + `reasoning-standard` + Market Historian & Strategist as real employees (Discovery + Case brief on the department). Containerized on a pinned Hermes version, behind the auth proxy (H1 spec), through the per-role delegation wrapper; capability sandbox + fencing (D20); skill promotion via the §8b promotion service. **Exit criteria: the §15 two-tenant isolation e2e + M3 network segmentation pass.**
3. Objective/Route/Waypoints on the department; Copywriter fan-out; **D29 run/approval lifecycle** end-to-end (persist → approve content-bound → new execution run).
4. Draft-review view + Simulator pre-flight + channel-class verified send + digest/edit-distance (D22) + **a minimal owned outcome-poller** (cron: API metrics on X/LinkedIn + click stats everywhere — the loop must not wait for stage 6's Social Media Manager). Community channels are clicks-plus-founder-reported channels by design (D19 reality) — the attribution UI says so honestly.
5. Memory graph (`MemoryNode`/`MemoryEdge` + `buildAgentContext` traversal, taint flags); the learning loop (feature-tagged attribution, evidence-weighted beliefs, explore/exploit); next-action recommender + Growth Analyst re-personalization. **Live graph + owned Timeline view only — graphify deferred to stage 7.**
