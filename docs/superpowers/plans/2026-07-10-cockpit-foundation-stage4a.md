...
- **Judgment calls on record:** no auth library (40 lines of node:crypto, all failure modes tested); CLI mints links (email ships later) with a deliberate 6-line mint duplication in .mjs; server actions return `{ok, message}` instead of throwing (UI-friendly, no stack traces); review reads are sequential loops (N is tiny at 4a — no premature query optimization); `next build` + direct handler invocation replace browser e2e at 4a.
  - GET-redeem CSRF/session-fixation posture (a live attacker-tenant link can overwrite a victim's session cookie via img-src; prefetchers can burn links): accepted at 4a (bounded impact — victim lands in the ATTACKER'S tenant, no victim-data exposure), MUST be mitigated (POST interstitial + refuse redemption over an existing valid session) before email delivery + high-stakes acts land (4d+).
  - Server-action result wiring is deliberately dormant at 4a: the drafts-page form closures discard ActionResult, so approve/reject failures are invisible in the UI; wiring (useActionState) + direct server-action tests are REQUIRED 4b work.
  - No session revocation at 4a (stateless HMAC cookie; secret rotation is the only kill-switch): acceptable for hand-provisioned partners, revisit with high-stakes acts.
